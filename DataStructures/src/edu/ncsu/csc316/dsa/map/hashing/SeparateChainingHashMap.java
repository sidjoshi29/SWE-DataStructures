...
//    	int oldSize = bucket.size();
    	V answer = table[hash].put(key, value);
//    	int n = n + (bucket.size() - oldSize);
    	size++;
    	return answer;
...
//    	SkipListMap<K, V> bucket = new SkipListMap<K, V>();
    	if(table[hash] == null || table[hash].get(key) == null) {
    		return null;
    	}
//    	int oldSize = bucket.size();
    	V answer = table[hash].remove(key);
//    	int n =- (bucket.size() - oldSize);
    	size--;
    	return answer;
    }
